...
- **Negative:** the gate protocol is one more shared reference to keep in sync; `build:11`/`fix:11` remain an ADR-009 manual byte-identical pair, now also mirrored by the `## GATE PROTOCOL` section. verify-all's shared-reference check enforces this: it byte-compares the two CRITICAL preambles, asserts the preamble still contains the closed-world prohibition + anti-laundering phrases (a phrase tripwire mirroring the F2 boundary content check — catching a *coordinated* revert of both preambles to an enumerated-only ban that byte-identity alone would miss), asserts the `## GATE PROTOCOL` section + `Read` directive exist in both pipelines, asserts the inline local-commit boundary is present in both Stage 8 sections, and guards against inline duplication of `gate-protocol.md`. (What it cannot check is whether the referenced prose stays semantically strong beyond those phrase tripwires — that remains a review responsibility; the closed-world rule and anti-laundering clause are also inlined into the always-loaded line-11 preamble so the safety-critical rules survive even if the shared `Read` is skipped or unresolved.)
