...
- **VERIFY:** whether making Stage 7 a blocking subagent interacts with the `--ci` path (build/fix `--ci` auto-proceeds gates). The subagent's `ESCALATE` under `--ci` must exit non-zero like other `--ci` cap conditions — confirm against the ADR-011/ADR-013 `--ci` contract.
