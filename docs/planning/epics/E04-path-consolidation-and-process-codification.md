...
1. **Plan-path migration dual-state surprise.** E04.S1 moves 16-ish runtime references across skills + 25 historical plans, plus adds a new `/roughly:upgrade` migration step. Risk: a skill is missed during the sweep and silently reads or writes to the old `docs/plans/` path post-merge, while the user's plans were already moved by `/roughly:upgrade` — producing a dual-state where the user's plans are at the new path but a skill still reads the old one (silent empty-file-on-missing-path per the E02.S2.3/S2.6 known pitfall). Compounding with v0.1.4's `.ruckus/` legacy state is the realistic residual: the migration check now has to identify two distinct pre-v0.1.6 legacy states. Mitigation: extend the existing pre-flight migration check pattern (proven across v0.1.4 + v0.1.5 — `rg -c "Legacy" skills/*/SKILL.md` returns 8 today) to a two-form check; blocking abort if either `.ruckus/` or `docs/plans/` is detected without the corresponding migrated state. CI dogfood asserts the post-migration state. Closes when E04.S1 ships and the CI assertion-path update lands green on `main`.

2. **Line-cap budget under additive pressure.** v0.1.5 end-state: `skills/build/SKILL.md` 298/300, `skills/fix/SKILL.md` 299/300 (1-line headroom on fix is binding). E04.S1 changes 16-ish runtime references in skills; most are path-string substitutions (net-zero), but Stage 3 plan-naming prose may need expansion to disambiguate `.roughly/plans/<feature>-plan.md` from the v0.1.5 convention, and E04.S3 (plan self-marking historical) adds Stage 7 prose. Risk: a story lands and pushes build or fix past 300, breaking the dogfood Stop hook (which enforces the cap at `verify-all.sh:25`). Mitigation: substitution-only discipline proven in S1, S9, S10 (each shipped within ±2 lines); if forced, the "refactor build/fix preamble + Stage 1 + Stage 8 into shared reference" candidate from E03's v0.1.6 list is the prepared off-ramp (same shared-reference pattern as `agents/agent-preamble.md` and ADR-003). Closes when all build/fix-touching stories ship without breach.
...
**Starting state (post-v0.1.5):** build 298/300, fix 299/300, setup 287/300, help 163/300, upgrade 164/300, review-plan 92/300, audit-epic 141/300, verify-all 80/300, review 87/300, review-epic 94/300.
...
- **AC4** — Each of the 7 hard-abort skill bodies (audit-epic, build, fix, review, review-plan, review-epic, verify-all) contains a two-form pre-flight check (`.ruckus/` legacy state OR `docs/plans/` legacy state). Verification: define a canonical two-form template block; `diff` each skill's pre-flight section against the canonical block must return zero. Setup's soft-abort form is the documented 8th and is explicitly excluded from the diff per the existing E03.S4 known-pitfall ("setup's soft-abort form must NOT be normalized to the canonical hard-abort form").
- **AC5** — `scripts/ci-dogfood.sh` assertion path updated (L153, L156 → `.roughly/plans/`); CI dogfood happy-path passes against the new path with the existing `--max-budget-usd 1.50` ceiling held. Verify: `rg -Fn "docs/plans" scripts/` returns zero matches.
...
- **AC2** — Block insertion is via `Edit` (not `Write`) per the known append-only pitfall — `old_string` is the file's current first line (the plan title); `new_string` is the Status block + blank line + the plan title. Verify: `git diff` on the plan file in the wrap-up commit shows only added lines, no `-` lines.
...
- **AC1.1** — New check block fires when the 7 hard-abort skill bodies' pre-flight migration check blocks (extended in E04.S1 to two-form) do not produce a single unique line under `sort -u`. The check scans the 7 named skill files only (audit-epic, build, fix, review, review-plan, review-epic, verify-all); setup is explicitly excluded per the E03.S4 known-pitfall ("setup's soft-abort form must NOT be normalized to the canonical hard-abort form").
...
- **AC3.1** — New check block fires when `.roughly/known-pitfalls.md` is at or above 80 lines (`wc -l` ≥ 80). Threshold matches the existing doc-writer Process step 5 conditional from E03.S3.
...
- **AC3.5** — Verification: confirm the check currently fires (known-pitfalls.md is 82/80 lines as of v0.1.5 release) — this is a known-good test case at story-merge time. Post-merge, the check will continue to fire until the next organize pass.
...
- **AC6** — `.claude/hooks/verify-all.sh` line count post-merge: documented in PR description for the line-cap budget tracking pattern (no formal cap, but worth recording).
...
For Check 3 specifically, the known-pitfalls.md 82/80-line state at story-merge time is itself the known-bad case — no separate broken-sample staging needed.
...
- **AC1 — ADR file created at `docs/adrs/ADR-011-skill-flags-as-public-api.md`:**
   - Format matches existing ADR-009 / ADR-008 structure.
   ...
   - **Consequences/Negative:** Flag proliferation risk on long-lived skills. Env-var-acceptable carve-out: debug-only, contributor-facing, no user-facing skill behavior change. Cites a hypothetical Haiku-routing budget threshold for cost-sensitive teams as an example of an env-var case that v0.2.0 might land.
...
- **AC1 — Multi-file failure-handling clause added.** `agents/doc-writer.md` Process step 5 (or the equivalent post-write step) gains an explicit branch: "If multi-file invocation: when both `.roughly/known-pitfalls.md` AND `CLAUDE.md` updates were dispatched in one invocation, perform each `Edit` independently and capture the per-file outcome. If any single `Edit` fails (non-zero return, parse error, file locked, permission denied), do not abort the other writes — complete the successful writes, then emit a partial-success summary that explicitly names which file(s) succeeded and which failed, including the failure reason from `Edit`'s error output. Never claim full success when a write partially failed." Wording is verbatim-locked in the AC; implementation must match.

- **AC2 — Strictly additive.** Verification: `git diff agents/doc-writer.md` on the PR shows zero `-` lines outside the addition site. Existing Process steps 1–4 and 6+ byte-identical pre/post-edit.
...
|---|---|---|---|---|
| 1 | **E04.S4** (dogfood `verify-all.sh` cleanup) | B | None | Smallest blast radius (latent bug fix); lands first so subsequent Stop-hook touches start from corrected base. Pair-ordering with S5 satisfied. |
| 2 | **E04.S6** (plan-discipline codification) | C | None | Process codification lands early so subsequent stories' `/roughly:review-plan` cycles inherit the new ACs. Carve-outs validate at first multi-branch downstream story (S5 or S2). |
| 3 | **E04.S7** (ADR-011) | C | None | ADR doc-only; fast PR. Lands before S1 so the skill-vs-env-var decision surface for S1's `--force-plans` flag is already codified. |
| 4 | **E04.S8** (doc-writer multi-file guard) | D | None | Land early to maximize the opportunistic Risk 5 close window. Real-dogfood multi-file invocations during v0.1.6's 5–6 week cycle close the risk; the longer the guard is in place, the better the chance of exercise. |
| 5 | **E04.S1** (path consolidation) | A | None (anchor) | The migration. CI assertion-path update lands in same PR. Post-merge CI passes against `.roughly/plans/`. |
...
| 8 | **E04.S5** (Stop hook drift expansion) | B | S1, S4 | Check 1 verifies the two-form pre-flight block from S1; Check 2 sits on S4's corrected `verify-all.sh` base. |
...
**Critical path:** S1 → S2 → (S5 if S4 not yet merged) → S3. ~5 sequential PRs minimum if everything else is parallelized; 9 PRs sequential if not.
...
- [ ] **All 9 stories merged** (S1 through S9 inclusive)
