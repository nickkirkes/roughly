...
- **`setup` installs the plan-mode-gate hook but writes no install marker** (E06.S6 discovery). `skills/setup/SKILL.md` Step 5b copies `plan-mode-gate.sh` into `.claude/hooks/` and registers it in `.claude/settings.json`, but never records a `plan-mode-gate-v1-added <date>` marker in `.roughly/workflow-upgrades` — unlike the maturity-check installs (`investigator-v1`, `stop-hook-v1`), which do record markers. Consequence: every freshly-set-up project has the plan-mode-gate hook installed but no marker for it, so E06.S6's new `/roughly:help` "Installed components" section will show nothing on a clean install (it only surfaces markers that exist in `workflow-upgrades`). E06.S6 was scoped to categorization plumbing only (per its out-of-scope list: "Adding new install markers"); the `plan-mode-gate-v1-added` marker was recorded **manually** in this dogfood repo's `workflow-upgrades` (E06.S6 T2) so AC1/AC2 manual verification could run. The categorization logic is correct and forward-compatible, but it has no auto-written marker to categorize on real installs until `setup` is taught to write one. **Mitigation candidate:** add a `plan-mode-gate-v1-added <date>` write to `skills/setup/SKILL.md` Step 5b (immediately after the hook copy + chmod succeed), mirroring the maturity-check marker-write pattern (Step 5d/5e), so the install marker is produced at install time. Consider whether `/roughly:upgrade` should back-fill the marker for projects that installed the hook before the write existed. Pairs with the broader question of whether other always-installed components (formatter hook, settings entries) should also gain install markers for `/roughly:help` visibility. **Defer rationale:** E06.S6's categorization plumbing ships independently and is the prerequisite; the marker-write is additive and low-risk but out of S6's scope. **Promote to v0.1.9 must-do** — without it, the Installed-components section is dead on arrival for any project that ran `setup` rather than hand-editing `workflow-upgrades`.
...
- **Epic AC verbatim-text back-annotation form** (E06.S1 cubic iteration, commits `7bad1f4` + `cd54a75`; reinforced by E06.S4 ship 2026-06-05 + E06.S5 dogfood-self ship 2026-06-08). When a build-pipeline cubic iteration retroactively modifies epic-verbatim AC text to resolve a real contradiction surfaced post-merge, the epic file needs a convention for recording what shipped vs what was specified. E06.S1 used an in-place "Post-merge deviation note" adjacent to the affected AC + commit references + canonical-shipped-form pointers to `agents/doc-writer.md` line numbers; this is the first instance of the pattern and was an ad-hoc choice. **Second instance:** E06.S4.AC1's marker-intent sub-carve-out (commits `97f6d8f` + `d7a658d`), where the principle was present in AC3 narrative but omitted from AC1's normative enumeration — same pattern: in-place adjacent deviation note + canonical-shipped-form pointer to `skills/review-plan/SKILL.md` L48–49 + `CONTRIBUTING.md` `## AC authoring conventions`. **Third instance (deferred-not-amended form):** E06.S5 Stage 6 surfaced W1 + W2 as spec-revision-candidates (above) — the new disposition prose itself dogfooded the new gate; deviations were classified rather than amended in-flight. This third instance demonstrates the alternative form: when a build-pipeline cubic iteration surfaces a real spec gap that AC text doesn't accommodate, EITHER (i) amend in-flight via the convention codified by this candidate (E06.S1 + E06.S4 form), OR (ii) classify as spec-revision-candidate + escalate to v0.1.X candidates (E06.S5 form using its own newly-shipped gate). Both are valid; the convention should cover both. **Distinct from "stale-line citations" candidate above** — that one covers external-reference rot in unmodified AC text; this one covers post-merge content modification of AC text. Options for codification: (a) in-place adjacent note (E06.S1 + E06.S4 precedent — preserves cubic-readability of the original AC text), (b) extend the cross-epic AC amendment convention to cover intra-epic build-time amendments (symmetry with E05.S3 / E06.S1.AC4), (c) separate CHANGELOG-only convention with epic-side cross-reference. Recommend (b) for convention symmetry, with the convention covering both in-flight amendment (i) and spec-revision-candidate escalation (ii). **Promotion priority: must-do for v0.1.9** given three-instance pattern across three stories in the same epic.
