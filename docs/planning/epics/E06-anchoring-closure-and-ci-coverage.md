...
- **Epic AC verbatim-text back-annotation form** (E06.S1 cubic iteration, commits `7bad1f4` + `cd54a75`; reinforced by E06.S4 ship 2026-06-05 + E06.S5 dogfood-self ship 2026-06-08 + E06.S2 ship 2026-06-08 + E06.S6 ship 2026-06-08). When a build-pipeline cubic iteration retroactively modifies epic-verbatim AC text to resolve a real contradiction surfaced post-merge, the epic file needs a convention for recording what shipped vs what was specified. E06.S1 used an in-place "Post-merge deviation note" adjacent to the affected AC + commit references + canonical-shipped-form pointers to `agents/doc-writer.md` line numbers; this is the first instance of the pattern and was an ad-hoc choice. **Cumulative shape catalog across five instances in E06:**
  ...
  - **Shape 4 — Premise-false spec / consumer-only-scope insufficient.** E06.S6.AC1 (consumer-side categorization specified against a data source that never had producer-side writes; would have shipped dead feature). Resolution: in-build scope expansion to include producer side (setup + upgrade) + adjacent deviation note.
  
  All four shapes share the property that the AC text as authored is technically intact but functionally misaligned with what shipped; the convention must cover all four (in-flight content amendment, escalation-and-defer, internal-contradiction scope-narrowing, in-build scope expansion). Options for codification: (a) in-place adjacent note (precedent: 4 of 5 instances — preserves cubic-readability of the original AC text), (b) extend the cross-epic AC amendment convention to cover intra-epic build-time amendments (symmetry with E05.S3 / E06.S1.AC4), (c) separate CHANGELOG-only convention with epic-side cross-reference. Recommend (b) for convention symmetry, with the convention covering all four shapes. **Distinct from "stale-line citations" candidate above** — that one covers external-reference rot in unmodified AC text; this one covers post-merge content/scope drift of AC text. **Promotion priority: must-do for v0.1.9** given five-instance pattern across four shipped stories in the same epic, with four distinct shapes documented.
