...
- **Epic AC verbatim-text back-annotation form** (E06.S1 cubic iteration, commits `7bad1f4` + `cd54a75`). When a build-pipeline cubic iteration retroactively modifies epic-verbatim AC text to resolve a real contradiction surfaced post-merge, the epic file needs a convention for recording what shipped vs what was specified. E06.S1 used an in-place "Post-merge deviation note" adjacent to the affected AC + commit references + canonical-shipped-form pointers to `agents/doc-writer.md` line numbers; this is the first instance of the pattern and was an ad-hoc choice. Options for codification: (a) in-place adjacent note (E06.S1 precedent — preserves cubic-readability of the original AC text), (b) extend the cross-epic AC amendment convention to cover intra-epic build-time amendments (symmetry with E05.S3 / E06.S1.AC4), (c) separate CHANGELOG-only convention with epic-side cross-reference. Recommend (b) for convention symmetry. Surface for v0.1.9 PM round.
