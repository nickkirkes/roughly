...
- **AC quoted-wording marker trigger-enumeration expansion** (SFH Warning W2, E06.S4 Stage 6). The shipped E06.S4.AC1 enumerated metasyntactic forms are `<placeholder>`, `…`, `XXX`, `<list>` — but the plugin's own template convention `{{PLACEHOLDER}}` (per CONTRIBUTING.md L47) and square-bracket fill-in tokens like `[N]` / `[finding]` / `[task ID]` (heavily used in `skills/review-plan/SKILL.md` output templates and ABORT HANDLING prose) are both structurally identical metasyntactic forms not enumerated. An AC quoting `"first line must be agent: partial success — [N] writes failed"` or `"emit {{PROJECT_NAME}} log lines"` would silently pass the check without a marker requirement. Mitigation candidate: extend the enumerated list in `skills/review-plan/SKILL.md` L48 and `CONTRIBUTING.md` L70 to include `{{PLACEHOLDER}}` and square-bracket `[text]` slots. **Disposition: spec-revision candidate (not a code defect).** Per the spec-revision-candidate pattern (codified by E06.S5 — upcoming in v0.1.8): the gap is in the shipped E06.S4.AC1 enumeration, not in the implementation. AC amendment via the cross-epic AC amendment convention is the correct vehicle. Defer rationale: surfaced at Stage 6 post-implementation; the convention as shipped covers the most common forms; expansion is additive and risk-free but requires AC amendment (no-edit-in-place rule applies). Promote to v0.1.9 must-do if any v0.1.8/v0.1.9 story author quotes `{{PLACEHOLDER}}` or `[text]` example wording without a marker and the gap allows the AC to ship.

**New from E06.S4 build Stage 6 silent-failure-hunter review (2026-06-05):**

- **AC quoted-wording marker — trigger-enumeration expansion** (SFH Warning W2, E06.S4 Stage 6 — disposition: spec-revision-candidate per E06.S5 pattern). The E06.S4-shipped check enumerates metasyntactic-notation triggers as `<placeholder>`, `…` (U+2026 ellipsis), `XXX`, `<list>`. Not enumerated: the plugin's own `{{PLACEHOLDER}}` template form (used in `CONTRIBUTING.md` L47 and `skills/setup/templates/`), square-bracket fill-in tokens (`[N]`, `[finding]`, `[task ID]` — heavily used in skill body output templates and agent return templates), and HTML-comment-internal metasyntax. An AC quoting any of these forms would currently fall under the bright-line carve-out (no enumerated metasyntactic notation = no marker required) when in fact they ARE metasyntactic and require marking. Promote to v0.1.9 must-do IF any story author quotes one of the unenumerated forms without a marker AND the gap allows the AC to ship without flag. Otherwise defer as opportunistic addition to the AC quoted-wording marker convention. Cost-bound: adding ~3 enumerated forms is a ~3–5 line edit to `skills/review-plan/SKILL.md` check L48–49 + matching update to CONTRIBUTING.md `## AC authoring conventions` subsection; well under cap.
...
- **Epic AC verbatim-text back-annotation form** (E06.S1 cubic iteration, commits `7bad1f4` + `cd54a75`; reinforced by E06.S4 ship 2026-06-05). When a build-pipeline cubic iteration retroactively modifies epic-verbatim AC text to resolve a real contradiction surfaced post-merge, the epic file needs a convention for recording what shipped vs what was specified. E06.S1 used an in-place "Post-merge deviation note" adjacent to the affected AC + commit references + canonical-shipped-form pointers to `agents/doc-writer.md` line numbers; this is the first instance of the pattern and was an ad-hoc choice. **Second instance:** E06.S4.AC1's marker-intent sub-carve-out (commits `97f6d8f` + `d7a658d`), where the principle was present in AC3 narrative but omitted from AC1's normative enumeration — same pattern: in-place adjacent deviation note + canonical-shipped-form pointer to `skills/review-plan/SKILL.md` L48–49 + `CONTRIBUTING.md` `## AC authoring conventions`. Two same-epic instances in two shipped stories raises the priority — the pattern is recurring, not one-off. Options for codification: (a) in-place adjacent note (E06.S1 + E06.S4 precedent — preserves cubic-readability of the original AC text), (b) extend the cross-epic AC amendment convention to cover intra-epic build-time amendments (symmetry with E05.S3 / E06.S1.AC4), (c) separate CHANGELOG-only convention with epic-side cross-reference. Recommend (b) for convention symmetry. **Promotion priority raised from "surface for v0.1.9 PM round" to "likely must-do for v0.1.9"** given two-instance pattern.
