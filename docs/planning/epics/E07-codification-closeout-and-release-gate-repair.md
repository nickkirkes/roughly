...
**Target version:** v0.1.9 · **Target effort:** ~6–8 days engineering + **0–6 paid dogfood dispatches** (~$0–$60 at the verified $10.05/run): 0–1 if E07.S2 hits BLOCKED-NO-EVIDENCE, 3 on a terminal CARRY, 4–6 on the success path. See S2.AC1 for the allocation and control flow. E07.S6.AC8 adds a separate ~$5 ceiling of local non-dogfood sessions. · **Status:** SCOPED — freezes v0.1.9. Revised 2026-07-30 per `/roughly:review-epic` ([review](E07-codification-closeout-and-release-gate-repair-review.md)); corrected 2026-08-10 (Phase 2 predecessor set, dispatch-budget allocation, harness invocation count, issue-brief refresh rule).
...
- **AC1 — E06 Risk 3: dispatch, iterate, then call it. The story runs in two separated phases.** *(Phase split added 2026-08-07; the AC previously read as one continuous dispatch sequence, which would have produced an invalid CLOSE — see the sequencing note below.)*
  ...
  **Discovery scope is bounded by class** (resolved at review 2026-07-30; supersedes an earlier unbounded "whatever the scenarios surface is in scope" framing, which authorized open-ended pipeline repair inside a close-out release):
  ...
  - **Class C — stage-logic regression in `skills/build/SKILL.md` or `skills/fix/SKILL.md`** (subagent dispatch, stage sequencing, Stage 5c cap semantics). **CARRY with a v0.1.10 stub, unless the human explicitly rescopes at that point.** Unbounded by nature; this is the class that breaches the ceiling. **Discovering Class C ends Phase 1 immediately** *(specified 2026-08-11)* — record the terminal CARRY and spend no further dispatches even if the cap is unspent, since retrying against an unfixed stage-logic regression buys nothing. Only an explicit human rescope reopens the budget, as a new decision.
  ...
  **Control flow between the phases** *(specified 2026-08-10 — the prior wording made Phase 1 exhaustion a CARRY trigger and Phase 2 "always funded" without saying whether Phase 2 still runs, leaving both the final disposition and the paid-dispatch count ambiguous)*:
  ...
  The **ceiling** stays 3 + 3 = 6 / ~$60 at the verified $10.05/run; actual spend is 3 on the CARRY path and 4–6 on the success path, per the control flow above. Class C is the only class with no retry allowance — it CARRIES by default precisely because it is unbounded, and rescoping it is a decision made with evidence rather than a commitment made here.
...
**Verification.** Paid, and the total depends on the outcome: **Phase 1 is capped at 3** (1 discovery + up to 2 re-dispatches for any IN-SCOPE class A/B/H). If it never reaches green, spend stops at **3 dispatches / ~$30** and the disposition is a terminal CARRY — Phase 2 does not run. If it reaches green, Phase 2's **3 reserved proving runs** follow, for **4–6 dispatches / ~$40–$60** at the verified $10.05/run — 4 when Phase 1 goes green on its first dispatch, 6 when it uses both re-dispatches. *(Corrected 2026-08-11 from a flat "6 / ~$60," which contradicted the 4–6 range the control-flow bullet already stated.)* See AC1 for the allocation and the control flow. **Each proving run records the `main` revision it ran against, plus its run ID and conclusion** *(added 2026-08-11)*. Phase 2 is only valid if all three ran against the **same** revision — the criterion is "no intervening change to the workflow or its inputs," and without the revisions written down, three greens spanning two different trees are indistinguishable from a qualifying sequence. **Take each revision from the workflow run itself, not from a local ref** *(corrected 2026-08-11)*: `gh run view <run-id> --json headSha,conclusion,createdAt`. A local `git rev-parse main` proves nothing about what a runner checked out — the local ref can be stale, ahead, or simply different from the SHA a run that started earlier is testing. Assert the three `headSha` values are identical and record them with their run IDs in the CHANGELOG line alongside the disposition. A CLOSE claimed without the three revisions recorded does not satisfy this AC — this is the same attestation-vs-artifact rule E07.S5 codifies, applied to the gate that decides the release.
...
- **AC8 — Executable behavioral check of the detector, run locally** *(added 2026-08-10; rewritten 2026-08-11 for internal consistency; rewritten again 2026-08-11 because the prescribed invocation could not perform the actions it asked for — see below)*. Desk-check alone leaves a user-visible prompt in both pipelines with no executable verification.

  **`claude -p` is single-turn.** It emits one reply and exits, so there is nothing to "abort" and no way to answer a gate. The earlier matrix told the operator to abort at prompts and answer a confirmation — neither is possible in print mode. *(This is precisely why `scripts/ci-dogfood.sh` drives the pipelines with `--ci` rather than answering gates.)* The arms are therefore split by what each mode can actually do:

  - **Arms 1–9 are print-mode observations.** The assertion is on **stdout**: does the gate's prompt text appear? The session ends on its own; no operator action, nothing to abort.
  - **Arm 10 is an interactive session** (`claude --plugin-dir .`, no `-p`), because answering a gate needs a second turn.

  **Resolve OQ8 first** — arms 1–4 derive from its accepted-forms enumeration.

  **Both pipelines, not just build** *(added 2026-08-11)*. AC1 requires equivalent behavior in `skills/build/SKILL.md` and `skills/fix/SKILL.md`, and the parity `diff` alone does not prove it: the insertion points differ structurally between the two files (build shares one sentence for resolution and `CI_MODE`; fix has them in separate paragraphs), so byte-identical prose can still behave differently. Fix-side arms are required.

  Every invocation: `--bare --plugin-dir . --no-session-persistence --max-budget-usd 0.50`.

  | # | Pipeline | Arm | Input | Required stdout |
  |---|---|---|---|---|
  | 1 | build | `#### E06.SN:` | `docs/planning/epics/complete/E06-anchoring-closure-and-ci-coverage.md` (7 headings) | prompt present |
  | 2 | build | `## E07.SN —` | the E07 epic file (7 headings) | prompt present |
  | 3 | build | `### E06.SN —` | `…-ci-coverage-audit.md` (7 headings) | prompt present |
  | 4 | build | `## Story N:` | scratch file from the README `### Epic files` example | prompt present |
  | 5 | build | story-ID enumeration | scratch file listing story IDs, no story-form headings | prompt present |
  | 6 | build | negative control | scratch file, single story | prompt **absent**; Stage 1 proceeds |
  | 7 | fix | heading form | any input from arms 1–4 | prompt present |
  | 8 | fix | story-ID enumeration | arm 5's input | prompt present |
  | 9 | fix | negative control | arm 6's input | prompt **absent**; Stage 1 proceeds |
  | 10 | build, **interactive** | confirm path | arm 1's input | prompt appears; answer with monolithic confirmation; Stage 1 then proceeds exactly as today |

  Ceiling **$5.00** across ten arms; actual well under $2, since every session stops at or just past Stage 1. Arms 1–4 are one per accepted heading form — add or drop rows to match OQ8 exactly, and **a form OQ8 declares accepted without a passing arm here fails this AC**. Arm 5 exists because AC1 specifies two triggers and a headings-only detector would otherwise pass everything while violating AC1. Arm 10 exists because arms 1–9 never exercise AC3's advisory-not-abort guarantee — they observe a prompt or its absence, never an answer.

  Scratch files only; do **not** commit a fixture. Record **each arm's** outcome in the CHANGELOG entry. **Local check, not a CI scenario** — nothing is added to `scripts/ci-dogfood.sh` or the dogfood budget.

- **AC9 — Durable regression coverage via a Stop-hook drift check** *(added 2026-08-11)*. AC8 is a one-off run at merge time; nothing then prevents a later edit from deleting or diverging the classifier, and the dogfood proving runs cannot catch it (they pass inline descriptions, so neither classification path executes). Add a check to `.claude/hooks/verify-all.sh` — and to `skills/setup/templates/verify-all-stop-hook.sh.template` if the check is generic enough to belong there — asserting that the Stage 1 intake-granularity block **exists in both pipeline files and is byte-identical between them**, using the same `issues="${issues}- …"` accumulation and the existing hook-pair byte-identity check as the pattern. This is structural coverage, not behavioral: it catches deletion and divergence, not a detector that silently stops matching. Say so in the CHANGELOG rather than implying the check closes the behavioral gap. The epic-input dogfood fixture stays a v0.1.10 candidate for the behavioral half.



**Verification.** Parity `diff` of the added Stage 1 blocks (use the same anchor-extraction approach S5.AC1 specifies); `wc -l` on all three skill files; the `### Epic files` check in AC4; the fix-side ordering check in AC1. For AC2 the guard is **`grep -c 'AskUserQuestion'` staying at exactly 1 in each of `skills/build/SKILL.md` and `skills/fix/SKILL.md`** — the verified baseline (both at L11). **This is a tripwire, not a proof** *(clarified 2026-08-11 — it was previously described as proving the closed-world property, which no grep can do: it catches only a re-mention of that one name, while prose enumerating any other structured or interactive tool passes it untouched).* It catches the single most likely regression. The closed-world property itself is verified by **reading the added prose** and confirming two things: it states the closed-world test, and it names no tool at all. Make that a named review step, not an inference from a passing grep. *(Previously stated as "unchanged from baseline" with the baseline never recorded, which made the check unrunnable and put it in direct tension with AC2's "state the constraint explicitly.")* Behavioral verification against a two-story fixture is desk-check only — no new paid CI scenario (adding one is out of scope per the epic's standing constraint). **Be explicit about what E07.S2's proving runs do and do not cover here** *(added 2026-08-10)*: all six dogfood scenarios pass an **inline description**, never a file, so **neither** classification path runs in CI — not the positive one and not the negative one, because the file-resolution branch AC1 hooks into is never reached. *(Corrected 2026-08-10: an earlier revision claimed the negative path was covered. It is not — inline input does not exercise resolved-file classification at all.)* What the proving runs **do** cover is that the added Stage 1 prose does not hang or otherwise break `--ci` on inline input: a gate inserted before the `CI_MODE` assignment would surface as a 124 timeout across every scenario. That is a real but narrow guarantee, and it is the only one they provide for this story. A classifier that fails to detect a genuine epic file, or that emits a malformed prompt when it does, ships on desk-check alone and three green proving runs would not catch it. **AC8 closes the executable-verification gap locally** — the positive path is exercised against a real epic file before merge, so the guard does not ship untested; what remains unclosed is *CI regression* coverage, which only a paid scenario would give. Record that residual in the CHANGELOG entry rather than letting green runs imply coverage they do not provide, and file the epic-input fixture as a v0.1.10 candidate (see `## v0.1.10 candidates`). `bash .claude/hooks/verify-all.sh` clean.
