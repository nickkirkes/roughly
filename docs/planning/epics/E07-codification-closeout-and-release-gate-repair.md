...
|---|---|---|---|---|
| E07.S3 — CHANGELOG backfill + heading revert | [#93](https://github.com/nickkirkes/roughly/issues/93) | `de54fe9` | 1 | Before every other story (rewrites the heading their entries land under) |
| E07.S1 — Dogfood harness diagnostics | [#94](https://github.com/nickkirkes/roughly/issues/94) | `de54fe9` | 2 | After S3 |
| E07.S2 **Phase 1** — discovery dispatch | [#95](https://github.com/nickkirkes/roughly/issues/95) | `de54fe9` | 3 | Opens once S1 merges; **runs concurrently with wave 4** |
| E07.S5 — Stage 4 contract | [#96](https://github.com/nickkirkes/roughly/issues/96) | `de54fe9` | 4 | After S3; before S6 (line budget, R3) |
| E07.S6 — Granularity guard + ADR-021 | [#97](https://github.com/nickkirkes/roughly/issues/97) | `de54fe9` | 4 | After S5 |
| E07.S4 — Intra-epic AC codifier | [#98](https://github.com/nickkirkes/roughly/issues/98) | `de54fe9` | 4 | Unordered within the wave; **optional** — first cut if the dispatch budget compresses |
| E07.S2 **Phase 2** — 3 proving runs | [#95](https://github.com/nickkirkes/roughly/issues/95) | `de54fe9` | 5 | After S1, S5, S6 have merged (AC1's predecessor set). S4 not required |
| E07.S7 — Tag-prep wrap | [#99](https://github.com/nickkirkes/roughly/issues/99) | `de54fe9` | 6 | Last; after S2's disposition is recorded |
...
- **AC8 — Executable behavioral check of the detector, run locally** *(added 2026-08-10 — desk-check alone left the guard's core path with no executable verification of any kind, in a story that ships a user-visible prompt to both pipelines)*. Documenting the CI coverage gap does not test the guard; run it. Two arms against a funded key, mirroring the S1.AC2 positive-plus-negative-control pattern:
  - **Positive:** `claude --bare --plugin-dir . -p "/roughly:build docs/planning/epics/complete/E06-anchoring-closure-and-ci-coverage.md"` — the classification prompt must fire. That file carries **seven** `#### E06.SN:` story headings. *(Path corrected 2026-08-10; the earlier example said `epics/complete/E0*.md`, which does not resolve from the repo root.)*
