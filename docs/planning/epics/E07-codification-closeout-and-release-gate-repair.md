...
**Dependencies:** E07.S1 (its diagnostics are what make a failed dispatch legible); E07.S3 for the AC5 surface-1 CHANGELOG lines. Should begin as soon as S1 merges — this is the long pole, and a dispatch that surfaces defects needs runway before tag.
...
- **AC2 — Shape catalog: transcribe E06's canonical Shapes 1–6, do not re-derive them.** *(Corrected at review 2026-07-30. The earlier framing — "E06 self-identifies four distinct shapes across five instances; the two remaining come from E06.S3" — was wrong: [E06 L601](complete/E06-anchoring-closure-and-ci-coverage.md) states* **"nine instances across all seven shipped stories, six distinct shapes"** *and enumerates them at L601–606; [ROADMAP.md:124](../../ROADMAP.md) repeats the 9/6 figures. The earlier derived list also dropped Shape 5 entirely and split Shape 4's sub-variant into a standalone shape.)* Adopt E06's catalog **with its numbering preserved**, so the codifier and its source agree:
  ...
  Each entry names the story, the shape, and the deviation note that records it. Verify: all six cite locations resolvable in the E06 epic file, and the catalog's shape count and instance count match E06 L601's stated 6 and 9.
...
```text
                    ...
                    │                                                            │
S3 CHANGELOG ───────┼──► S4 codifier ─────────────────────────────────────────────┤──► S7 tag-prep
  (heading revert)  │                                                            │
...
**Critical path is S3 → S1 → S2 → S7.** S2 is the long pole and the only story with an external dependency (paid dispatches) and a discovery component. Start S1 immediately after S3 and S2 the moment S1 merges — a dispatch that surfaces defects in scenarios dark for seven weeks needs runway, and running it at tag crunch is how the tag slips.

**Track B and C still gate nothing functionally.** S4 and S5→S6 are parallel with each other and with the critical path once S3 has landed. Their ordering constraints are: S3 before all (CHANGELOG), S5 before S6 (line budget, R3), and all before S7. If the release window compresses they can merge in any order; if S2's discovery run consumes the budget, S4 is the first cut — it is dogfooding-internal with no consumer-facing effect. S2 and S4 both write the E06 epic file in different sections; expect a rebase, not a conflict.
