...
- **AC8 — Executable behavioral check of the detector, run locally** *(added 2026-08-10; rewritten whole 2026-08-11 after incremental edits left it internally inconsistent — it said "two arms" while requiring five invocations, told the operator to abort in a control arm that produces no prompt, tested only one of four heading forms, tested neither the story-ID-enumeration trigger nor the confirm path, and set no spend cap)*. Desk-check alone leaves a user-visible prompt in both pipelines with no executable verification, and documenting the CI coverage gap does not test the guard.

  **Resolve OQ8 first.** The accepted-forms set is what makes these arms derivable; an arm list cannot be written before that answer exists.

  **Every invocation carries `--max-budget-usd 0.50`** — the check reaches Stage 1 only, so the cap is a backstop, not a target. Form: `claude --bare --plugin-dir . --no-session-persistence --max-budget-usd 0.50 -p "/roughly:build <input>"`. With six arms the hard ceiling is **$3.00**; expected actual is well under $1 since each session aborts or proceeds at Stage 1.

  | # | Arm | Input | Required outcome | Operator action |
  |---|---|---|---|---|
  | 1 | Heading form `#### E06.SN:` | `docs/planning/epics/complete/E06-anchoring-closure-and-ci-coverage.md` (7 headings) | prompt **fires** | abort |
  | 2 | Heading form `## E07.SN —` | this epic file (7 headings) | prompt **fires** | abort |
  | 3 | Heading form `### E06.SN —` | `docs/planning/epics/complete/E06-anchoring-closure-and-ci-coverage-audit.md` (7 headings) | prompt **fires** | abort |
  | 4 | Heading form `## Story N:` | scratch file built from the README `### Epic files` example — the form the README teaches, so the one a consumer is most likely to write | prompt **fires** | abort |
  | 5 | **Story-ID enumeration** (AC1's second trigger) | scratch file listing story IDs with no story-form headings | prompt **fires** | abort |
  | 6 | **Negative control** | scratch file with a single story (README Quick Start heredoc) | prompt does **not** fire; Stage 1 proceeds exactly as today | none — there is nothing to abort at; let Stage 1 proceed and confirm it does |

  Arms 1–4 are one per accepted heading form; drop or add rows to match OQ8's enumeration exactly. **A form declared accepted in OQ8 without a passing arm here fails this AC**, and any form the heuristic does not accept must be a named exclusion in OQ8 with a reason, never silently unsupported. **Arm 5 exists because AC1 specifies two triggers** — recognized headings *or* an explicit story-ID enumeration — and a detector handling only the first would otherwise pass every arm while violating AC1.

  **Arm 7 — the confirm path.** Re-run arm 1 and answer the prompt with monolithic confirmation instead of aborting. Stage 1 must then proceed exactly as it does today. This is AC3's advisory-not-abort guarantee, and arms 1–6 never exercise it: they either abort at the prompt or never see one. Abort at the next gate. Same `--max-budget-usd 0.50`, bringing the ceiling to **$3.50**.

  Scratch files only — do **not** commit a fixture. Record **each arm's outcome** in the CHANGELOG entry, naming the input and whether the prompt fired. **This is a local check, not a CI scenario:** nothing is added to `scripts/ci-dogfood.sh` or the dogfood budget, so the no-new-paid-scenarios constraint holds.
