...
|---|---|---|---|---|
| E07.S3 — CHANGELOG backfill + heading revert | [#93](https://github.com/nickkirkes/roughly/issues/93) | `533f81d` | 1 | Before every other story (rewrites the heading their entries land under) |
| E07.S1 — Dogfood harness diagnostics | [#94](https://github.com/nickkirkes/roughly/issues/94) | `533f81d` | 2 | After S3 |
| E07.S2 **Phase 1** — discovery dispatch | [#95](https://github.com/nickkirkes/roughly/issues/95) | `533f81d` | 3 | Opens once S1 merges; **runs concurrently with wave 4** |
| E07.S5 — Stage 4 contract | [#96](https://github.com/nickkirkes/roughly/issues/96) | `533f81d` | 4 | After S3; before S6 (line budget, R3) |
| E07.S6 — Granularity guard + ADR-021 | [#97](https://github.com/nickkirkes/roughly/issues/97) | `533f81d` | 4 | After S5 |
| E07.S4 — Intra-epic AC codifier | [#98](https://github.com/nickkirkes/roughly/issues/98) | `533f81d` | 4 | Unordered within the wave; **optional** — first cut if the dispatch budget compresses |
| E07.S2 **Phase 2** — 3 proving runs | [#95](https://github.com/nickkirkes/roughly/issues/95) | `533f81d` | 5 | After S1, S5, S6 have merged (AC1's predecessor set). S4 not required |
| E07.S7 — Tag-prep wrap | [#99](https://github.com/nickkirkes/roughly/issues/99) | `533f81d` | 6 | Last; after S2's disposition is recorded |

Deferred: [#100](https://github.com/nickkirkes/roughly/issues/100) (B3, v0.1.10) · [#101](https://github.com/nickkirkes/roughly/issues/101) (DI-001, v0.2.0-adjacent) · [#102](https://github.com/nickkirkes/roughly/issues/102) (ADR-009/010 cleanup, v0.2.0) — all three briefs derived from `533f81d`, same as the story rows above.
...
- **AC1 — E06 Risk 3: dispatch, iterate, then call it. The story runs in two separated phases.** *(Phase split added 2026-08-07; the AC previously read as one continuous dispatch sequence, which would have produced an invalid CLOSE — see the sequencing note below.)*
  ...
  **BLOCKED-NO-EVIDENCE: stopping rule, evidence, and where it can land** *(specified 2026-08-11 — the disposition previously said only "key depletion mid-story," which left a block occurring after Phase 1 had already spent dispatches with no spend total, no stopping rule, and no branch at all for a reserved Phase 2 run that cannot execute)*:
  ...
  - **Declared when** a dispatch terminates on S1.AC1's `ci-dogfood: FAIL — API account state: <classification>` marker, or when `gh workflow run` fails to create a run.
  ...
  - **Evidence is the same as for any other run** — the failed run's ID, URL, `headSha`, and the account-state marker text; or, if no run was created, the `gh workflow run` error verbatim. Dispatches already made keep their Phase 1 record (see Verification); the block does not erase them.
...
git diff --quiet <first> <last> -- \
  ...
  skills/build/SKILL.md skills/fix/SKILL.md \
  $(grep -ho 'skills/shared/[a-z0-9-]*\.md' skills/build/SKILL.md skills/fix/SKILL.md | sort -u)
```
...
- **AC9 — Durable regression coverage via a Stop-hook drift check** *(added 2026-08-11; rewritten same day — as drafted it was unsatisfiable whenever AC7's extraction off-ramp was taken, and it did not enforce the ordering AC1 exists to guarantee)*. AC8 is a one-off run at merge time; nothing then prevents a later edit from deleting, diverging, or **relocating** the classifier, and the dogfood proving runs cannot catch it (they pass inline descriptions, so neither classification path executes). Add the checks below to `.claude/hooks/verify-all.sh`, using the existing `issues="${issues}- …"` accumulation and the plan-mode-gate hook-pair check as the pattern. Free — no session, no dispatch.
  ...
  **Check B — ordering, enforcing AC1's positional requirement.** Byte-identity does not stop a later edit from moving the block **above** the `CI_MODE=true` assignment, which hangs every `--ci` run at the new gate — precisely the failure AC1's positional insertion point exists to prevent, and one no parity `diff` can see. Assert, for **both** files (build's resolution and `CI_MODE` share one sentence today, but a later split would make ordering load-bearing there too), that the anchor's line number is **greater than** the `CI_MODE=true` assignment's:
  ...
  for f in skills/build/SKILL.md skills/fix/SKILL.md; do
    ...
    if [ -z "$ci" ] || [ -z "$cls" ] || [ "$cls" -le "$ci" ]; then
      issues="${issues}- intake-granularity ordering: $f classifier at line ${cls:-?} is not after CI_MODE=true at line ${ci:-?} (AC1 positional insertion point — a gate before the assignment hangs every --ci run)\n"
    fi
  done
  ```
