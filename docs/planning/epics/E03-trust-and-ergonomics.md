...
**Status:** **v0.1.5 release-prep complete (2026-05-13); awaiting tag push.** 13/13 implementation stories merged + S12.0 decision resolved. **All three clusters DONE:** trust-hardening 7/7 (S0–S6); CI 3/3 (S11a, S11b-1, S11b-2); ergonomics 3/3 (S8, S9, S10). PRs #24 (S0), #25 (S1), #26 (S3), #27 (S2), #28 (S5), #29 (S4), #30 (S6), #31 (S11a), #32 (S11b-1), #33 (S11b-2), #34 (S10), #35 (S9), #36 (S8, `740ab5f`, merged 2026-05-12). E03.S12.0 resolved 2026-05-08 via option (c) defer; docs cluster (S12a/S12b) moved to v0.1.6 candidates. OQ1 resolved 2026-05-08 (`--ci` flag); OQ3 ratified 2026-05-10 and shipped via S10 with Path C. Final line counts: [skills/build/SKILL.md](../../../skills/build/SKILL.md) 298/300; [skills/fix/SKILL.md](../../../skills/fix/SKILL.md) 299/300; new [skills/help/SKILL.md](../../../skills/help/SKILL.md) 163/300; [.roughly/known-pitfalls.md](../../../.roughly/known-pitfalls.md) at 82/80 (post-audit; over the organize threshold — doc-writer's S3 organize-suggestion will fire on next pipeline write). **No prose-extraction off-ramp invoked across the entire release** — every additive story stayed within budget via inline substitution or compression. **Release-prep shipped** 2026-05-13 via `f0182df` (plugin.json 0.1.4 → 0.1.5; CHANGELOG heading `[Unreleased]` → `[0.1.5] — 2026-05-13`; ROADMAP `**Current:** v0.1.5` + remaining items marked Done/Deferred). **Audit verdict** ([E03-trust-and-ergonomics-audit.md](E03-trust-and-ergonomics-audit.md), PR #37 merged 2026-05-13): **91 MET / 5 PARTIAL / 0 NOT MET across 96 ACs**; cross-cutting consistency PASS; zero regressions; shipping-ready. Doc-hygiene fixes applied inline at S1 AC6 (L171), S2 AC5 (L260), S3 AC6 (L323); new regex-metachar-hazard pitfall captured during the audit-fix Stage 4 review-plan. **Remaining release task:** `git tag v0.1.5 && git push origin v0.1.5` + configure `ANTHROPIC_API_KEY` GitHub repo secret.
...
**Status:** Complete on `feat/E03.S9-abort-prose`; merged 2026-05-12 via PR #35 (`2a4dfdc`, 4 commits — `e0cecd7` core sweep + `015bb4d` 2 wrap-up pitfalls + `fe2a6ad` Stage 5c file-state field fix + `d9b624e` plan verify-command count correction). All 6 ACs met. **Per-site inline substitution** chosen over template block — template would have cost ≥2 lines in fix's 1-line headroom. **Net-zero line delta** across all 3 modified skills; no prose-extraction off-ramp invoked across the entire release.

**Files delivered:**

Modified:
- [skills/build/SKILL.md](../../../skills/build/SKILL.md) — 298 → 298 (+0). 13 substitutions at gate prompts + escalation/cap sites (including the L185 catch-all decision-table summary).
- [skills/fix/SKILL.md](../../../skills/fix/SKILL.md) — 299 → 299 (+0). 13 parallel substitutions matching build.
- [skills/review-plan/SKILL.md](../../../skills/review-plan/SKILL.md) — 92 (no line change). 1 substitution at L21: NEEDS REVISION return now starts with `Stage 4 review-plan cannot proceed:` and ends with `Recovery: run /roughly:setup first.`
- [.roughly/known-pitfalls.md](../../../.roughly/known-pitfalls.md) — 72 → 76 (+4). Two new pitfalls in Build & Deploy section (commit `015bb4d`).

Verified out of scope (zero edits):
- [skills/review-epic/SKILL.md](../../../skills/review-epic/SKILL.md) — no in-pipeline abort gates
- [skills/audit-epic/SKILL.md](../../../skills/audit-epic/SKILL.md) — same

New:
- `docs/plans/E03-S9-abort-prose-plan.md` — implementation plan artifact
...
**Acceptance criteria:**
- [x] Every abort branch in build/fix/review-plan produces a 4-field message (stage + reason + file state + recovery). 27 branches updated total; review-epic and audit-epic verified out of scope.
- [x] **Positive verification:** regex pattern `Stage .* (aborted|stopped|cannot proceed)` matches at every site (build 13 / fix 13 / review-plan 1); recovery markers `recovery|next step|re-run|escalate` present (15 / 15 / 2).
- [x] **Negative verification:** `rg -n 'aborted\b' skills/ | rg -v 'Stage'` returns zero hits.
- [x] **ABORT HANDLING block byte-verbatim.** MD5 verified at build L276-298 / fix L277-299 between HEAD and the post-merge tree — zero changes.
- [x] **Template block MAY be used** — chose per-site inline substitution; template would have cost ≥2 lines in fix's 1-line headroom (AC framed as "MAY", not "MUST").
- [x] **Line-cap budget contract** — build 298, fix 299, both unchanged from pre-edit.

**Verification (delivered):**
- Stage 4 plan review: PASS
- Stage 6 (3-agent parallel review): cycle 1 caught two convergent findings — stranded `escalate to human` at build L185 / fix L192 (decision-table summary lines missed by T2/T4 because the plan flagged them as "confirm during edit" footnotes rather than numbered sites; both code-reviewer and silent-failure-hunter flagged independently); and double-period at backtick boundaries (build L180 / fix L187 — `fix.\.` produced an ambiguous emit-string boundary). Both fixed in a single review-fix cycle. Cycle 2: PASS / CLEAN from all three review agents.
- Post-merge cubic review surfaced two further issues fixed in follow-up commits: 3 Stage 5c emit strings per file (question-loop, out-of-scope, spec-compliance) initially omitted the file-state field while 3 others included it — semantic AC violation that Stage 6 reviewers PASSed because the AC required file state but not a literal `Files:` prefix. Fixed in `fe2a6ad` (6 edits). And T2/T4 verify-command counts in the plan conflated `aborted` and `cannot proceed` as one category — caught by cubic post-merge, fixed in `d9b624e`.

**Two pitfalls captured during wrap-up (commit `015bb4d`):**
1. **Decision-table summary lines are discrete emit sites, not restatements** — when sweeping a "If X: A. If Y: B. If Z: C." block, the trailing summary lines must be enumerated as separately numbered edit sites in the plan, not left as "confirm during edit" footnotes.
2. **Inline backtick-quoted emit strings followed by additional prose take exactly one period — inside the backtick.** Placing periods both inside and outside the closing backtick produces an ambiguous boundary for LLM parsing.
...
- **Plan-path consolidation: `docs/plans/` → `.roughly/plans/` (raised 2026-05-13).** Move implementation-plan storage under `.roughly/` so all Roughly-managed state lives in one root. Motivation: (1) naming collision with user's `docs/planning/` for epics; (2) PR review tools flag committed plan files as noise; (3) users who want to gitignore Roughly state can ignore `.roughly/` as a unit. **Decisions locked 2026-05-13:** v0.1.6 timing; blocking pre-flight abort across 6 pipeline skills (avoids dual-path-forever); `git status --porcelain` safety check in `/roughly:upgrade` for uncommitted plan work (with `--force-plans` opt-in). Scope: 16 runtime references + 24 historical plans (`git mv` per E02.S2.6 precedent) + new ~3-point `/roughly:upgrade` migration step (mirrors v0.1.4 `.ruckus/` 10-point spec, simplified because plans are read-only) + CI assertion path update + README/CONTRIBUTING/CHANGELOG + possibly ADR-011. Estimated effort: 7-10 hours. Natural seed for v0.1.6 epic "path conventions + upgrade hardening" bundle (pairs with the other path-conventions items already in this list — plan-file self-marking historical, marker-aware resume, dogfood verify-all.sh cleanup). Scoping notes captured in [.claude/plans/read-docs-planning-prompts-roughly-pm-ag-curious-cloud.md](../../../../.claude/plans/read-docs-planning-prompts-roughly-pm-ag-curious-cloud.md) (local).
