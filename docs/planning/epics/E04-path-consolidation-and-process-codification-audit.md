...
**Quality notes:** AC5's literal verify command is self-defeating in the same way AC1's was — S1 already documented this pattern at epic L609 ("Pre-flight `rg -Fn` verify-command self-defeat pattern") as v0.1.7 candidate. The audit re-confirms the pattern affects AC5 too, not just AC1. **Recommendation:** retroactively edit AC5's verify command to `rg -Fn "docs/plans" scripts/ README.md CONTRIBUTING.md | grep -vi "migrat" | grep -v "retro-mark"` (case-insensitive partial match on "migrat" — covers L87's lowercase "migrate"/"migration"; the original capital-M anchor would miss). Fold into the v0.1.7 codification of self-defeating-verify-pattern review-plan AC for systematic enforcement. **Updated 2026-05-24:** initial recommendation used `grep -v "Migration"` (case-sensitive); fixed post-audit-PR cubic review.
