...
Per AC5: this plan's body was reviewed by `/roughly:review-plan` at Stage 4 with the post-E05.S3 checks active (E05.S3.AC1–AC5 + E05.S6.AC2 + E04.S6.AC1–AC2). The post-E06.S4 marker check was not yet shipped at plan-review time, so the plan body was not evaluated against it (the new check activates for downstream stories per epic L321). Stage 4 returned PASS in 1 iteration with no concerns or blockers.

**Post-merge correction (cubic P2, 2026-06-04):** an earlier version of this section asserted a "fenced-code-block carve-out" — that the plan's quoted AC text inside fenced code blocks was exempt from the marker requirement by markdown convention, with implicit `form:` semantics. That carve-out is not part of the shipped marker rule. The shipped carve-outs in `skills/review-plan/SKILL.md` L49 are exactly two: (a) the bright-line carve-out for quoted wording containing no metasyntactic notation, and (b) the marker-intent sub-carve-out for close-but-not-identical marker names with unambiguous adjacent prose. Fenced-code-block context confers no implicit marker semantics. The earlier prose has been removed; future readers should treat the shipped SKILL.md and CONTRIBUTING.md text as the only normative sources for the marker convention.
