...
2. **T2 marker clarification.** T2's numbered list renders the five template `Read` targets as shorthand (`…/<file>.template`). Those *display* entries are illustrative (`form:`), not literal; the concrete `verbatim:` target applied at each site is the full `${CLAUDE_PLUGIN_ROOT}/skills/setup/templates/<file>.template`, and T2's Verify command asserted that concrete anchored form — so implementation was unambiguous despite the shorthand. Full internal self-consistency would have required the display list to use literal paths matching the `verbatim:` Details sentence (marking only the shorthand `form:` would still contradict that sentence); and the Verify's prefix-count (`=6`) is weaker than a per-filename assertion. Both are plan-authoring niceties, not correctness gaps: post-hoc verification confirmed all six concrete templates — `CLAUDE.md.template`, `known-pitfalls.md.template`, `claudeignore.template`, `settings.json.template`, `verify-all-stop-hook.sh.template`, `plan-mode-gate.sh.template` — are each anchored exactly once with zero bare remaining.
