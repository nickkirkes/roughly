...
1. **Detection and safety check:** If `docs/plans/` does not exist, skip this step entirely (idempotent). Otherwise check `git status --porcelain docs/plans/ 2>/dev/null` — if non-empty (uncommitted edits to historical plans), abort with: `"Uncommitted changes in docs/plans/. Commit or stash them, or pass --force-plans to override."` If `--force-plans` appears as a standalone token in `$ARGUMENTS` (preceded by whitespace or string start, followed by whitespace or string end — not as a substring of `--force-plans-dry-run` or similar), proceed despite dirty status. Also check whether `.roughly/plans/` already exists with any content other than a `.migration-in-progress` marker — if so, abort with: `"Both docs/plans/ and .roughly/plans/ exist with content. Resolve manually (review both, choose one location, remove the other) before re-running."` (`.roughly/plans/` containing ONLY the marker is the resume case from a prior failed attempt — `git mv` cannot move into a non-empty destination, so manually clear `.roughly/plans/` first, preserving the marker if you want resume semantics.) Detect git availability with `git rev-parse --git-dir 2>/dev/null` — silent failure means non-git; use plain `mv` otherwise use `git mv` (preserves history per E02.S2.6 precedent).

2. **Move:** Write marker at `docs/plans/.migration-in-progress` (the SOURCE dir — writing into the destination would create `.roughly/plans/` and make `git mv` fail because the destination would already exist) containing the current ISO date and plugin version, then perform `git mv docs/plans/ .roughly/plans/` inside git (or plain `mv` otherwise) — the directory rename carries the marker along to `.roughly/plans/.migration-in-progress` atomically (matches v0.1.4's marker-at-source idiom). If the move command returns non-zero, surface the error verbatim and abort — the marker stays at `docs/plans/.migration-in-progress` for re-run. Do NOT fall back between `git mv` and `mv` on failure (post-failure recovery via the other tool produces confusing error output that complicates the user's mental model of which tool moved what — inherits v0.1.4's idiom).
