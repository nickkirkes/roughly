...
Plan files persist in `docs/plans/` after their feature ships — there is no machine-readable in-progress marker, and recency is only a hint (a paused-and-resumed feature may have an old timestamp). The display surfaces the most-recent plans and lets the user identify which (if any) is current; nothing is filtered out as "archived" by mtime alone.

**Zero matches:** emit `"No plan files found in docs/plans/."` and skip the rest of Step 3.

**Exactly one match:** emit:
> "One plan in docs/plans/:
> - `<filename>` (modified <timestamp from ls -lt, verbatim>)"

No user prompt — single match is unambiguous.

**Multiple matches:** Show up to the 10 most-recently-modified plans inline. If the total exceeds 10, append a count-of-older line so paused work is discoverable.

Emit:
> "Plan files in docs/plans/ (most recent first; older plans persist after their feature ships):"

Then list up to 10 entries:
> - `<filename>` (modified <timestamp from ls -lt, verbatim>)
> - ...

If `total > 10`, also emit (with `N = total - 10`):
> "(<N> older plans not listed. Answer `all` to expand the full list, `none` if no pipeline is in progress, or paste a filename above for your current plan.)"

If `total <= 10`, also emit:
> "(Paste a filename above for your current plan, or answer `none` if no pipeline is in progress.)"

Then ask: **"Which plan is current?"**

Do NOT silently pick the most-recent file. Do NOT filter or guess based on filename. Display the entries the user can choose from and wait for their answer.

Trim leading and trailing whitespace from the user's response before matching. Then evaluate (case-insensitive for `all`/`none`):
- Response is `all` AND `total > 10` AND the full list has not yet been shown this turn → re-emit the full `ls -lt` list (no cap), then re-ask the same question. Treat any further `all` response as invalid (the expansion is one-shot).
- Response is `none` → emit: `"No active pipeline. Old plans can be deleted manually when no longer useful."` and end Step 3.
- Response exactly matches one of the listed filenames (the visible list, plus the full list if `all` was used) → emit: `"Current plan: <filename>. Resume with /roughly:build or /roughly:fix as appropriate."` and end Step 3.
- Anything else → emit: `"That filename was not in the list above. Paste one of the displayed filenames exactly, or answer 'none'."` then re-display the currently-visible list and re-ask. Do NOT fuzzy-match.

After three invalid responses, emit: `"Could not identify a current plan after three attempts. Continuing without selecting a plan."` and end Step 3 without picking. This bounds the loop so help never blocks the session.
