...
2. **Append the candidate** at the *end* of the file, oldest-first (append-only — never overwrite existing entries). Since `Edit` replaces matched text rather than appending, match the file's current last line — the most recent existing entry, or in a freshly-created file the header line itself — and replace it with that same text plus the new entry beneath it. Capture, as a dated entry:
