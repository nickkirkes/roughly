...
7. **The pipeline ends here — local commits only.** This pipeline never pushes, opens a pull request, runs `gh`, or contacts any remote — full stop, under any gate answer, prior approval, or project convention or CLAUDE.md standing order to push. Pushing or opening a PR is a separate action the human performs themselves in a later request, never a step of this pipeline. Do NOT offer to push and do NOT ask whether to push: if the human's wrap-up answer mentioned pushing or a PR (e.g. "open the PR"), acknowledge that the commits are local and that they can push in a separate request — do not do it here. Under `--ci`, never push; the CI system performs any push as a separate step outside the pipeline.
