...
- **AC quoted-wording marker.** When an AC quotes example wording that contains metasyntactic notation (angle-bracketed placeholders like `<placeholder>`, ellipses `…`, or sample-text tokens like `XXX` / `<list>`), the AC must explicitly mark whether the wording is `verbatim: <text>` (text must match literally byte-for-byte at implementation) or `form: <text> (adapt for clarity)` (text is illustrative of structure; implementer chooses concrete wording within the structural form). ACs that quote metasyntactic-containing example wording WITHOUT a marker are flagged for clarification before approving the plan.
  - **Carve-out:** ACs whose quoted wording contains NO metasyntactic notation (plain literal text only — fixed strings used in `grep -Fn` patterns, exact section headings, etc.) pass without requiring a marker. Canonical positive: an AC quoting `` "confirm your summary's first line is `prefix — …`" `` (the trailing `…` is metasyntactic per the `.roughly/known-pitfalls.md` § "Skill & Agent Authoring" "LLM self-check anchor pattern" entry) — required `form:` marker (would have prevented the E05.S2.AC2(c) anchoring-wording deferral). Canonical negative: an AC saying `` grep -Fn "Cross-epic AC amendments" CONTRIBUTING.md `` returns ≥1 match — quoted text is verbatim search literal with no metasyntactic notation; no marker required (passes). **Marker-intent sub-carve-out:** a close-but-not-identical marker name (e.g., `literal:` instead of `verbatim:`, or `template:` instead of `form:`) with adjacent prose that unambiguously expresses verbatim-required intent (e.g., "must match byte-for-byte at implementation") or illustrative-only intent (e.g., "adapt for clarity") passes — the label is illustrative, the inline prose is normative. The check fires on missing-intent (no marker AND no unambiguous prose), not on label-non-conformance alone.

- **AC executable-example verify command.** When an AC's normative text embeds executable example content (a shell command, code snippet, or structured-data literal presented as something to run or paste), its verify command must exercise the example's *specific* invocation — confirm each flag/subcommand it uses appears in `<command> --help` output, run it with a dry-run/no-op flag, or syntax-check it. A bare `<command> --help` exit-0 check is NOT sufficient (it proves only the base command exists, not that the example's arguments are well-formed). ACs that embed an executable example but carry no verify validating its grammar are flagged for clarification before approving the plan. Canonical positive: E06.S7.AC1's `gh api PATCH /repos/<owner>/<repo>/pulls/<pr>` example (HTTP method as a positional instead of `--method`/`-X`) shipped with no grammar-validating verify — Stage 6 checked intent and cross-references but never ran it; the malformed command was caught only by external post-merge review (2026-06-09).
  - **Carve-out:** illustrative or pseudocode snippets not presented as literally runnable (schematic templates, prose-with-inline-code, `<placeholder>`-heavy usage sketches) are out of scope — the check fires only on an example an implementer would copy-paste and run. Canonical negative: an AC quoting `` grep -Fn "Cross-epic AC amendments" CONTRIBUTING.md `` as a verbatim search literal, or a `<owner>/<repo>`-templated usage sketch — not a concrete runnable invocation, so no execution-verify is required.

- **Mirror-verbatim vs negative-grep cross-test.** When an AC combines a "verbatim copy from X" requirement (its clause must mirror source X byte-for-byte) with a negative-grep verify clause (a byte sequence must NOT appear), cross-test the two before approving the plan: locate X's literal text and test the negative grep's forbidden byte sequence directly against it. If X itself contains that byte sequence, a true verbatim copy fails the AC's own verify — flag for clarification. Resolution is either (a) show the verbatim form passes the negative grep, or (b) scope "verbatim" to a load-bearing grep-safe sub-clause of X.
  - **Carve-out:** the trigger is byte-level containment, NOT file identity — the check fires whenever the byte sequence the negative grep forbids is a literal substring of X's verbatim text, even when X's file and the grep's target file differ (they did in the incident below). It does NOT fire when an AC carries only one of the two clauses, or when the forbidden sequence is absent from X. Canonical positive: E06.S2.AC1 — a "verbatim from build-side reference" requirement (X = the flag-detection sentence in `skills/build/SKILL.md`) paired with a negative grep on a *different* file (`skills/fix/SKILL.md`) forbidding a byte sequence that X's sentence literally contains; a true verbatim copy failed the AC's own negative grep (resolved by scoping "verbatim" to the load-bearing parenthetical rather than the full sentence). Canonical negative: an AC mirroring `skills/build/SKILL.md`'s Stage 8 prose verbatim while a separate negative grep forbids an unrelated string never present in that prose — the forbidden sequence is not a substring of X, so no collision and the check does not fire.
...
- **Runtime-signal source named.** Any task that performs runtime detection (mtime, branch name, file content, JSON field, command output) MUST name the observable signal source — the specific command, file path, or field whose output the conditional reads. A conditional that does not name its data source is unverifiable. Canonical positive example: E03.S10's "if the failure output indicates a test failure — assertion errors or test-runner output" (`skills/build/SKILL.md` L180, `skills/fix/SKILL.md` L187). Canonical negative example: E03.S10 first-draft "if Stage 5c was hit by changes to test files" — no detection mechanism for "test files" exists (`.roughly/plans/E03-S10-retry-loop-tuning-plan.md`; commit `3c46687`).
  - **Carve-out:** this is a correctness check (where data comes from), NOT a maintenance check. Policy parameters — thresholds, comparators, target values — are explicitly out of scope. A duplicated `300` threshold is not a violation of this check.
...
- **AC joint satisfiability.** For each pair of ACs in the plan that reference overlapping surfaces (same file, same step, same prose region), verify joint satisfiability. If two ACs create a structural impossibility within the plan's implementation scope, flag for clarification before approving the plan.
  - **Carve-out:** ACs that reference orthogonal surfaces (different files AND different steps AND different prose regions) skip this check. Any single shared dimension (same file even if different steps, same step even if different prose regions, etc.) keeps the check live. Canonical positive: E04.S8's AC2/AC4-vs-AC1-reachability contradiction — AC2 forbade modification outside Stage 5 AND AC1 required the modification to fire when Stage 5's gate was closed; joint impossibility surfaced only post-merge during cubic iterations. Canonical negative: a plan whose ACs touch different files with no shared step or prose region — orthogonal across all three dimensions, skip.
