...
**`--ci` non-interactive Stage 4:** If `CI_MODE=true`: dispatch review-plan as above (do NOT skip it), then act on the verdict without a human gate — on PASS, auto-progress to Stage 5; on NEEDS REVISION, emit structured `FAIL — plan-review NEEDS REVISION: <verdict block>` and halt — that marker is the CI signal (no revision loop, no human gate). Build's `--ci` also runs review-plan (ADR-013, superseding its earlier skip-and-synthesize) but recovers from NEEDS REVISION via a revision loop (up to 2 verdicts); fix's `--ci` halts on the first NEEDS REVISION — a known asymmetry, slated for unification review in v0.1.9 (see E06 epic v0.1.9 candidates).

**If NEEDS REVISION (interactive mode only — the `CI_MODE=true` path halts on the marker above without entering this loop):** apply the suggested edits to the plan file, then re-dispatch the review-plan subagent against the updated plan. Repeat until PASS or until 2 total NEEDS REVISION verdicts — then escalate: emit `Stage 4 plan-review cannot proceed: 2 NEEDS REVISION verdicts. Plan at [path] needs human revision. Recovery: revise plan or override.` and present findings to the human.
